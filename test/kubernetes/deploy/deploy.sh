...
readonly DEFAULT_PLUGIN_IMAGE='edeckers/do-csi-luks-plugin:dev'
...
kustomize edit set image edeckers/do-csi-luks-plugin="${DEV_IMAGE}"
# Undo any image updates done to kustomization.yaml to prevent git pollution.
# shellcheck disable=SC2064
trap "kustomize edit set image edeckers/do-csi-luks-plugin=$DEFAULT_PLUGIN_IMAGE" EXIT

# Apply the customization to the dev manifest, and apply it to the cluster.
kustomize build . --load_restrictor none | kubectl apply -f -
# Wait for the deployment to complete.
kubectl -n kube-system wait --timeout=5m --for=condition=Ready pod -l app=csi-do-luks-controller-dev
kubectl -n kube-system wait --timeout=5m --for=condition=Ready pod -l app=csi-do-luks-node-dev
